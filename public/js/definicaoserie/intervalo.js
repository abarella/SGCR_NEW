...
window.definirSerieAtividadeTable = function(event) {
    ...
    const tipo = 1;
    const inicio = document.getElementById('cmbAtvIni').value;
    const fim = document.getElementById('cmbAtvFim').value;
    ...
    fetch('/dfv-ds/definir-serie-intervalo', {
        method: 'POST',
        headers: {
    ...
    .catch(error => {
        ...
        alert("Erro ao definir a série por atividade. Tente novamente.");
    });
};
...
window.validarCampos = function() {
    ...
    // Validar se Atividade Final é maior que Inicial
    if (parseFloat(fim) <= parseFloat(inicio)) {
        alert("A ATIVIDADE FINAL DEVE SER MAIOR QUE A INICIAL!");
        return false;
    }
